    ...
    public class InvoiceRepository
    {
        ...
        public Invoice GetInvoice(int id, string userId)
        {
            using (var context = new ApplicationDbContext())
            {
                return context.Invoices
                    ...
                    .Single(x => x.Id == id && UserId == userId);
        ...
        public void UpdatePosition(InvoicePosition invoicePosition, string userId)
        {
            using (var context = new ApplicationDbContext())
            {
                ...
                positionToUpdate.Value = positionToUpdate.Product.Value
        ...
        public void DeletePosition(int id, string userId)
        {
            using (var context = new ApplicationDbContext())
            {
                var positionToDelete = context.Invoices
                       .Single(x => x.Id == id && x.UserId == userId);
                context.Invoices.Remove(positionToDelete);
                context.SaveChanges();
            }
        }
    }
